...
13. EstadoREDCOM (String)
    - Posibles Estados: ["0 - Pendiente de arrancar","1 - En curso","2 - Terminado","3 - Cancelado","4 - Cerrado","5 - Archivado"]
14. Cliente (String)
15. CategoríaISO (String)
    - "CategoríaISO": ["1 - Desarrollo de software","2 - Integración de sistemas","3 - Venta de licencias","4 - Iniciativas internas"]
16. ProjectCost (Decimal)
17. ProjectPercentCompleted (Int)
...
- If a company or client is mentioned, filter by the field `Cliente`. 
...
- Use datetime 'YYYY-MM-DDTHH:MM:SS' for dates in OData v3 filters. Valid example: FechaInicio ge datetime'2025-04-01T00:00:00'.
...
system: /Projects?$format=json&$select=ProjectName,ProjectManagerName,EstadoREDCOM,Cliente,CategoríaISO&$orderby=ProjectName&$filter=3
user: most expensive internal initiative 
system: /Projects?$format=json&$filter=CategoríaISO eq '4 - Internal Initiatives'&$orderby=ProjectActualCost desc&$select=ProjectName,ProjectActualCost,EstadoREDCOM,Client&$top=1
user: show me all projects
system: /Projects?$format=json&$select=ProjectName,EstadoREDCOM,Cliente,CategoríaISO
""",
...
5. FechaaltaREDCOM (DateTime)
6. FechabajaREDCOM (DateTime)
7. ResourceEmailAddress (String)
8. ResourceDepartments (String)
9. ResourceType (Int16)
10. ResourceStandardRate (Decimal)
11. ResourceIsGeneric (Boolean)
12. ResourceEarliestAvailableFrom (DateTime)
13. ResourceCostPerUse (Decimal)
...
Valid example: FechaInicio ge datetime '2025-04-01T00:00:00'. 
- IMPORTANT: Always use substringof('value', Field) to search by project name, 
